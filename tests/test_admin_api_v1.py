...
def test_admin_api_meta_contract_shape_permissions_and_determinism(monkeypatch):
    ...
    assert set(data["capabilities"]) == {"admin_api_v1", "read_api", "write_api", "technical_api_actions", "decision_card", "risk_attention", "report_export", "workflow_state_read", "workflow_actions_execute"}
    ...
    assert {"review_status", "human_verdict", "next_action", "outcome_status", "agent_task_status", "source_type", "verification_status", "workflow_state", "workflow_action", "decision_recommendation", "risk_category", "risk_severity"} == set(data["enums"])
