...
PR28 adds a deterministic sale and cap-rate evidence read model (`SALE_EVIDENCE_MODEL_VERSION = "v0"`, `CAP_RATE_EVIDENCE_MODEL_VERSION = "v0"`). It reads only already structured fields from the active PR25-selected and PR24-usable market evidence set, reusing PR27 source-quality caps locally. It does not retrieve new evidence, mutate evidence rows, backfill old analyses, call LLM/agents/RAG, scrape pages, geocode, or externally verify sale evidence.

The model writes compact `sale_and_cap_rate_evidence` facts under market evidence facts. These facts summarize explicit sale/asking sale fields and explicit structured `cap_rate_pct` only. `cap_rate_pct` uses percentage units (`8.2` means `8.2%`) and is rejected rather than clamped when outside documented plausibility bounds. RUB is the only supported currency in v0; missing or non-RUB currency is reviewed/excluded.

PR28 is descriptive decision-support evidence only. It does not estimate fair value, calculate target object value, produce target-equivalent sale price, perform appraisal, run DCF, or perform financing/tax/scenario modeling. Median observed price per m2 is an evidence summary only and is not multiplied by target area. Gross yield remains distinct from cap rate and derived gross yield is disabled by default in v0. Manual rent remains primary, PR26 adjusted rent values/medians are unchanged, and investment score/verdict are not directly rewritten by PR28.
