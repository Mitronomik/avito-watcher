...
Counters are derived only from returned items. `total_count` excludes `not_applicable` items. `checked_count` means evaluated checks with status `ok` or `warning`, not passed checks. `critical_missing_count` and `blocking_item_count` count critical items with status `missing` or `blocked`. `not_applicable` items do not hurt readiness and are not counted in `total_count`.
