...
<div class='note'>Freshness safety: the first run initializes baseline and does not alert. Use max_age_hours to avoid old publications. Use missing_published_at_policy carefully; reject is safest. If using allow_when_date_sorted, make sure the source URL is sorted by date and set Source sort=date.</div>
